...
public class JsonParsing : MonoBehaviour
{
    ...
    public void parseJsonfile(string playerName)
    {
        if (File.Exists(this.jsonfilePath))
        {
            ...
            foreach (PlayerInformation playerInformation in playerInfoList.model)
            {
                ...
                if (playerInformation.cid.Equals(playerName))
                {
                    PhotonNetwork.NickName = playerInformation.name;

                    RoomManager.text_Level1 = playerInformation.info_level_1;
                    RoomManager.text_Level2 = playerInformation.info_level_2;
                    RoomManager.text_Level3 = playerInformation.info_level_3;
    ...
    public string getPlayerName(string cid)
    {
        string name = "";
        foreach (PlayerInformation playerInformation in playerInfoList.model)
        {
            if (playerInformation.cid.Equals(cid))
            {
